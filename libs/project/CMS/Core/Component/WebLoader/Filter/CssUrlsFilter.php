...
class CssUrlsFilter
{
	...
	{
		$this->docRoot = \CMS\Core\Component\WebLoader\Path::normalize($docRoot);

		if (!is_dir($this->docRoot)) {
			throw new \CMS\Core\Component\WebLoader\InvalidArgumentException('Given document root is not directory.');
		}
	...
	public function absolutizeUrl($url, $quote, $cssFile)
	{
		...
		$cssFile = \CMS\Core\Component\WebLoader\Path::normalize($cssFile);
	 ...
	 * Invoke filter
	 * @param string $code
	 * @param \CMS\Core\Component\WebLoader\Compiler $loader
	 * @param string $file
	 * @return string
	 */
