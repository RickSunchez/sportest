...
class Compiler
{
	 ...
	 * @return Compiler
	 */
	public static function createCssCompiler(IFileCollection $files, $outputDir,$prefix = null)
	{
		return new static($files, DefaultOutputNamingConvention::createCssConvention($prefix), $outputDir);
	}
	 ...
	 * @return Compiler
	 */
	public static function createJsCompiler(IFileCollection $files, $outputDir,$prefix = null)
	{
		return new static($files, DefaultOutputNamingConvention::createJsConvention($prefix), $outputDir);
	}
	...
	protected function generateFiles(array $files, $ifModified)
	{
		...
		if (!$ifModified || !file_exists($path) || $lastModified > filemtime($path)) {
            ...
            if (function_exists("gzopen") && $env->getParameters('webloader.gzip.init')) {
                $gz = gzopen($outPath . '.' . $env->getParameters('webloader.gzip.ext'), 'wb9');
                gzwrite($gz, $content);
                gzclose($gz);
            }
		}
	...
	/**
	 * @return \CMS\Core\Component\WebLoader\IFileCollection
	 */
	...
	/**
	 * @return \CMS\Core\Component\WebLoader\IOutputNamingConvention
	 */
	...
	/**
	 * @param \CMS\Core\Component\WebLoader\IFileCollection $collection
	 */
	...
	/**
	 * @param \CMS\Core\Component\WebLoader\IOutputNamingConvention $namingConvention
	 */
