...
class Upload {
    ...
    function upload($file, $lang = 'en_GB') {
        ...
        if (is_string($file)) {
        ...
        }elseif(is_object($file) && $file instanceof \Delorius\Http\FileUpload ){
            // this is an element from Delorius\Http\Fileupload, i.e. an uploaded file
            $this->log .= '<b>source is an uploaded file</b><br />';
            if ($this->uploaded) {
                $this->file_src_error         = $file->getError();
                switch($this->file_src_error) {
                    case UPLOAD_ERR_OK:
                        // all is OK
                        $this->log .= '- upload OK<br />';
            ...
            if ($this->uploaded) {
                $this->file_src_pathname   = $file->getTemporaryFile();
                $this->file_src_name       = $file->getName();
            ...
            if ($this->uploaded) {
                ...
                $this->file_src_size = $file->getSize();
                $mime_from_browser = $file->getType();
            }
        }
    ...
    function process($server_path = null) {
        ...
        if ($this->processed) {
            ...
            if ($this->file_safe_name) { // formats the name
                $this->file_dst_name_body = utf8_encode(strtr(utf8_decode($this->file_dst_name_body), utf8_decode('Х ХНХЁХОХИУУУУУУУУУУУУУУУУУУУУУУУУУУУУ УЁУЂУЃУЄУЅУЇУЈУЉУЊУЋУЌУ­УЎУЏУБУВУГУДУЕУЖУИУЙУКУЛУМУНУП'), 'SZszYAAAAAACEEEEIIIINOOOOOOUUUUYaaaaaaceeeeiiiinoooooouuuuyy'));
                $this->file_dst_name_body = strtr($this->file_dst_name_body, array('У' => 'TH', 'УО' => 'th', 'У' => 'DH', 'УА' => 'dh', 'У' => 'ss', 'Х' => 'OE', 'Х' => 'oe', 'У' => 'AE', 'УІ' => 'ae', 'ТЕ' => 'u'));
            ...
            if ($image_manipulation) {
                ...
                if ($this->processed && $image_src) {
                    ...
                    if ($gd_version >= 2 && $this->image_unsharp && is_numeric($this->image_unsharp_amount) && is_numeric($this->image_unsharp_radius) && is_numeric($this->image_unsharp_threshold)) {
                        ...
                        // Unsharp mask algorithm by Torstein HУИnsi 2003-07. 
