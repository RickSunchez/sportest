...
class Method extends Object
{
    ...
    public function __toString()
    {
        ...
        . ($this->abstract || $this->body === FALSE ? ';'
            : ($this->name ? "\n" : ' ') . "{\n" . Strings::indent(ltrim(rtrim($this->body) . "\n"), 1) . '}');
    }
    ...
    public function setParameters(array $val)
    {
        ...
        foreach ($val as $v) {
            if (!$v instanceof Parameter) {
                throw new InvalidArgument('Argument must be Delorius\PhpGenerator\Parameter[].');
            }
     ...
     * @return self
     */
    public function addBodyClass($object)
    {
		...
		Class: ?
		*********/';
        $this->body .= Helpers::formatArgs($statement, array($class)) . "\n";
        return $this;
    }
    ...
    public function setVisibility($val)
    {
        if (!in_array($val, array('public', 'protected', 'private', NULL), TRUE)) {
            throw new InvalidArgument('Argument must be public|protected|private|NULL.');
        }
