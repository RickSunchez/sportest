...
class Helpers
{
	...
	private static function _dump(& $var, $level = 0)
	{
		if ($var instanceof PhpLiteral) {
			...
			if (empty($var)) {
				...
				throw new InvalidArgument('Nesting level too deep or recursive dependency.');
			...
			throw new InvalidArgument('Cannot dump closure.');

		} elseif (is_object($var)) {
			if (PHP_VERSION_ID >= 70000 && ($rc = new \ReflectionObject($var)) && $rc->isAnonymous()) {
				throw new InvalidArgument('Cannot dump anonymous class.');
			}
			$arr = (array) $var;
			$space = str_repeat("\t", $level);
			$class = get_class($var);
			...
			throw new InvalidArgument('Cannot dump resource.');
	...
	public static function formatArgs($statement, array $args)
	{
		...
		while ($a !== FALSE) {
			if (!$args) {
				throw new InvalidArgument('Insufficient number of arguments.');
			}
			$arg = array_shift($args);
			if (substr($statement, $a + 1, 1) === '*') { // ?*
				if (!is_array($arg)) {
					throw new InvalidArgument('Argument must be an array.');
				}
