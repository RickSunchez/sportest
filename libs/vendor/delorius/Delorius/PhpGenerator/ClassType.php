...
class ClassType extends Object
{
	...
	public function setType($type)
	{
		if (!in_array($type, array('class', 'interface', 'trait'), TRUE)) {
			throw new InvalidArgument('Argument must be class|interface|trait.');
		}
		$this->type = $type;
		return $this;
	}
	...
	public function setExtends($types)
	{
		if (!is_string($types) && !(is_array($types) && array_filter($types, 'is_string') === $types)) {
			throw new InvalidArgument('Argument must be string or string[].');
		}
		$this->extends = $types;
		return $this;
	}
	...
	public function setProperties(array $props)
	{
		...
		foreach ($props as $v) {
			if (!$v instanceof Property) {
				throw new InvalidArgument('Argument must be Delorius\PhpGenerator\Property[].');
			}
	...
	public function getProperty($name)
	{
		if (!isset($this->properties[$name])) {
			throw new InvalidArgument("Property '$name' not found.");
		}
	...
	public function setMethods(array $methods)
	{
		...
		foreach ($methods as $v) {
			if (!$v instanceof Method) {
				throw new InvalidArgument('Argument must be Deloriues\PhpGenerator\Method[].');
			}
	...
	public function getMethod($name)
	{
		if (!isset($this->methods[$name])) {
			throw new InvalidArgument("Method '$name' not found.");
		}
