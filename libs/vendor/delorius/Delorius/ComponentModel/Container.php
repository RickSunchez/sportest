...
use Delorius\Iterators\InstanceFilter;
...
class Container extends Component implements IContainer
{
	...
	public function addComponent(IComponent $component, $name, $insertBefore = NULL)
	{
		...
		if (is_int($name)) {
			...
			throw new Error("Component name must be integer or string, " . gettype($name) . " given.");
	...
	final public function getComponent($name, $need = TRUE)
	{
		...
		if (isset($this->components[$name])) {
			if (!isset($ext)) {
				...
				throw new Error("Component with name '$name' is not container and cannot have '$ext' component.");
			}
	...
	protected function createComponent($name)
	{
		...
		if ($ucname !== $name && method_exists($this, $method) && $this->getReflection()->getMethod($method)->getName() === $method) {
			...
			if (!$component instanceof IComponent && !isset($this->components[$name])) {
				...
				throw new Error("Method $class::$method() did not return or create the desired component.");
			}
			return $component;
		}
	}
	...
	final public function getComponents($deep = FALSE, $filterType = NULL)
	{
		...
		if ($filterType) {
			$iterator = new InstanceFilter($iterator, $filterType);
		}
		return $iterator;
	}

	/**
	 * Descendant can override this method to disallow insert a child by throwing an Error
	 * @return void
	 * @throws Error
	 */
