...
class MySQL extends DataBase {
	...
	public function connect()
	{
		...
		try
		{
			...
			if ($persistent)
			{
				...
				$this->_connection = @mysqli_connect('p:'.$hostname,$username,$password,$database,$port);
			}
			else
			{
				// Create a connection and force it to be a new link
			    $this->_connection = @mysqli_connect($hostname,$username,$password,$database,$port);
			}

            if(!$this->_connection){
                throw new \Exception('Cannot connect by MySQL Server');
            }
		}
		catch (\Exception $e)
		{
	...
	protected function _select_db($database)
	{
		if ( ! mysqli_select_db($this->_connection,$database))
		{
			...
			throw new DataBaseError(mysqli_error($this->_connection));
		}
	...
	public function query($type, $sql, $as_object = FALSE, array $params = NULL)
	{
		...
		if ( ! empty($this->_config['connection']['persistent']) AND $this->_config['connection']['database'] !== MySQL::$_current_database[$this->_connection_id])
		{
		...
		if (($result = mysqli_query($this->_connection,$sql)) === FALSE)
		{
			...
			throw new Error(mysqli_error($this->_connection)." [ $sql ] ");
		}
		...
		if ($type === DataBase::SELECT)
		{
			...
			return new \Delorius\DataBase\MySQL\Result($result, $sql, $as_object, $params);
		}
	...
	public function escape($value)
	{
        ...
        {
            throw new Error(mysqli_error($this->_connection));
        }
