...
final class SessionSection extends Object implements \IteratorAggregate, \ArrayAccess
{
	...
	{
		if (!is_string($name)) {
			throw new Error ("Session namespace must be a string, " . gettype($name) ." given.");
		}
	...
	private function start()
	{
		if ($this->meta === FALSE) {
			$this->session->start();
			$this->data = & $_SESSION['__DF']['DATA'][$this->name];
			$this->meta = & $_SESSION['__DF']['META'][$this->name];
		}
	}
	...
	public function __set($name, $value)
	{
		...
		if (is_object($value)) {
            $this->meta[$name]['V'] = ClassType::from($value)->getAnnotation('serializationVersion');
        }
	}
