<?php
namespace Delorius\Loaders;

use Delorius\Caching\Cache;
use Delorius\Caching\IStorage;
use Delorius\Caching\Storage\DevNullStorage;
...
/**
 * Delorius auto loader is responsible for loading classes and interfaces.
 *
 * @property-read array $indexedClasses
 * @property   \Delorius\Caching\IStorage $cacheStorage
 */
class RobotLoader extends Object
{
	...
	public function __construct()
	{
		if (!extension_loaded('tokenizer')) {
			throw new NotSupported('PHP extension Tokenizer is not loaded.');
		}
	}
	...
	public function rebuild()
	{
		...
		$this->getCache()->save($this->getKey(), Callback::closure($this, 'rebuildCallback'));
	}
		...
		foreach ($this->scanPaths as $path) {
			foreach (is_file($path) ? array(new \SplFileInfo($path)) : $this->createFileIterator($path) as $file) {
				...
				foreach ($classes as $class) {
					...
					if (isset($info['file'])) {
						throw new InvalidState("Ambiguous class $class resolution; defined in {$info['file']} and in $file.");
					}
	...
	/**
	 * Creates an iterator scaning directory for PHP files, subdirectories and 'dfrobots.txt' files.
	 * @return \Iterator
	...
	{
		if (!is_dir($dir)) {
			throw new Error("File or directory '$dir' not found.");
		}
		...
		$iterator = Finder::findFiles(is_array($this->acceptFiles) ? $this->acceptFiles : preg_split('#[,\s]+#', $this->acceptFiles))
			...
			->filter($filter = function (\SplFileInfo $dir) use (& $disallow) {
				...
				if (is_file("$path/dfrobots.txt")) {
					foreach (file("$path/dfrobots.txt") as $s) {
	...
	private function scanPhp($code)
	{
		...
		if (preg_match('#//delorius'.'loader=(\S*)#', $code, $matches)) {
	...
	protected function getCache()
	{
		...
		return new Cache($this->cacheStorage, 'Delorius.RobotLoader');
	}
