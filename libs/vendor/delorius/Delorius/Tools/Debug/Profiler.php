...
class Profiler extends Object
{
    ...
    public static function start($group, $name)
    {
        ...
        $token = 'dp/' . base_convert($counter++, 10, 32);
    ...
    public static function application()
    {
        ...
        /** @var \Delorius\Caching\Cache $cache */
        $cache = Environment::getContext()->getService('cache')->derive('profile');

        $stats = $cache->load('profiler_application_stats');
        ...
        $time = microtime(TRUE) - DELORIUS_START_TIME;

        // Get the total memory usage
        $memory = memory_get_usage() - DELORIUS_START_MEMORY;
        ...
        // Cache the new stats
        $cache->save('profiler_application_stats', $stats, array(
            Cache::EXPIRE => '+1 day',
            Cache::TAGS => array('profile')
        ));
