...
use Delorius\Tools\Math\Exceptions\DivisionByZeroException;
use Delorius\Tools\Math\Exceptions\OutOfScopeException;
use Delorius\Tools\Math\Exceptions\UnknownTokenException;
use Delorius\Tools\Math\Contexts\Scope as Scope_;
...
class Scope implements IfContext
{
    ...
    public function handleToken($token)
    {
        ...
        if (in_array($token, $this->supportedOperations, true)) {
            ...
            $this->builder->pushContext(new Scope_\Sin($token));
        } elseif ($token === 'acos(') {
            $this->builder->pushContext(new Scope_\Acos($token));
        } elseif ($token === 'cos(') {
            $this->builder->pushContext(new Scope_\Cosin($token));
        } elseif ($token === 'sum(') {
            $this->builder->pushContext(new Scope_\Sum($token));
        } elseif ($token === 'avg(') {
            $this->builder->pushContext(new Scope_\Avg($token));
        } elseif ($token === 'tan(') {
            $this->builder->pushContext(new Scope_\Tangent($token));
        } elseif ($token === 'sqrt(') {
            $this->builder->pushContext(new Scope_\Sqrt($token));
        } elseif ($token === 'log(' || $token === 'ln(') {
            $this->builder->pushContext(new Scope_\Log($token));
        } elseif ($token === 'pow(') {
            $this->builder->pushContext(new Scope_\Pow($token));
        } elseif ($token === 'if(') {
            $this->builder->pushContext(new Scope_\IfElse($token));
        } elseif ($token === 'exp(') {
            $this->builder->pushContext(new Scope_\Exp($token));
        } else {
            if (is_numeric($token)) {
