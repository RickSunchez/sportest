...
class SelectBox extends BaseControl
{
	...
	public function setItems(array $items, $useKeys = TRUE)
	{
		...
		foreach ($items as $k => $v) {
			foreach ((is_array($v) ? $v : array($k => $v)) as $key => $value) {
				if (!$useKeys) {
					if (!is_scalar($value)) {
						throw new Error("All items must be scalar.");
					}
					$key = $value;
				}

				if (isset($allowed[$key])) {
					throw new Error("Items contain duplication for key '$key'.");
				}
	...
	public function getControl()
	{
		...
		if ($this->prompt !== FALSE) {
			$control->add($this->prompt instanceof \Delorius\View\Html
				? $this->prompt->value('')
		...
		foreach ($this->items as $key => $value) {
			...
			foreach ($value as $key2 => $value2) {
				if ($value2 instanceof \Delorius\View\Html) {
