 ...
 *
 * <code>
 * $image = Image::fromFile('delorius.jpg');
...
class Image extends Object
{
	...
	public static function fromFile($file, & $format = NULL)
	{
		...
		default:
			die("Unknown image type or file '$file' not found.");
		}
	}
	...
	public static function fromBlank($width, $height, $color = NULL)
	{
		...
		$height = (int) $height;
		if ($width < 1 || $height < 1) {
			die('Image width and height must be greater than zero.');
		}
	...
	public static function calculateSize($srcWidth, $srcHeight, $newWidth, $newHeight, $flags = self::FIT)
	{
		...
		if ($flags & self::STRETCH) { // non-proportional
			if (empty($newWidth) || empty($newHeight)) {
				die('For stretching must be both width and height specified.');
			}
			...
			if (empty($newWidth) && empty($newHeight)) {
				die('At least width or height must be specified.');
			}
