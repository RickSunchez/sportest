...
/**
 * Delorius\Core\Object behaviour mixin.
 *
 */
final class ObjectMixin
{
    ...
    {
        throw new  Error('Not use ObjectMixin::__construct()');
    }
    ...
    public static function call($_this, $name, $args)
    {
        ...
        if ($name === '') {
            throw new Error("Call to class '$class' method without name.");
            ...
            if (is_array($_this->$name) || $_this->$name instanceof \Traversable) {
                ...
                throw new Error("Property $class::$$name must be array or NULL, " . gettype($_this->$name) ." given.");
            }

        } elseif (isset($methods[$name]) && is_array($methods[$name])) { // magic @methods
            list($op, $rp, $type) = $methods[$name];
            if (sizeof($args) !== ($op === 'get' ? 0 : 1)) {
                throw new Error("$class::$name() expects " . ($op === 'get' ? 'no' : '1') . ' argument, ' . sizeof($args) . ' given.');

            } elseif ($type && $args && !self::checkType($args[0], $type)) {
                throw new Error("Argument passed to $class::$name() must be $type, " . gettype($args[0]) . ' given.');
            }
    ...
    {
        throw new Error("Call to undefined static method $class::$method().");
    }
    ...
    public static function & get($_this, $name)
    {
        ...
        if ($name === '') {
            throw new Error("Cannot read a class '$class' property without name.");
            ...
            throw new Error("Cannot read $type property $class::\$$name.");
        }
    }
    ...
    public static function set($_this, $name, $value)
    {
        ...
        if ($name === '') {
            throw new Error("Cannot write to a class '$class' property without name.");

        } elseif (self::hasProperty($class, $name)) { // unsetted property
            $_this->$name = $value;

        } elseif (isset($methods[$class][$m = 'set' . $uname])) { // property setter
            $_this->$m($value);

        } else { // strict class
            $type = isset($methods['get' . $uname]) || isset($methods['is' . $uname])
                ? 'a read-only' : 'an undeclared';
            throw new Error("Cannot write to $type property $class::\$$name.");
        }
    }
