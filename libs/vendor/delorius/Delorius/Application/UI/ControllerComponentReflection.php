 ...
 * @internal
 */
class ControllerComponentReflection extends ClassType
{
	...
	public function getPersistentParams($class = NULL)
	{
		...
		$params = array();
		if (is_subclass_of($class, 'Delorius\Application\UI\Control')) {
	...
	public function hasCallableMethod($method)
	{
		...
		if ($cache === NULL) try {
			$cache = FALSE;
			$rm = \Delorius\Reflection\Method::from($class, $method);
		...
		foreach ($method->getParameters() as $param) {
			...
			if (isset($args[$name])) { // NULLs are ignored
				...
				if (!self::convertType($res[$i-1], $type)) {
					...
					throw new Error("Invalid value for parameter '$name' in method $mName(), expected " . ($type === 'NULL' ? 'scalar' : $type) . ".");
				}
			} else {
