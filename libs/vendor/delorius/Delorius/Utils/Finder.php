 ...
 *
    ...
    foreach (Finder::findFiles('*.txt')->exclude('_temp_')
    ->from($dir) as $file) {
    }
...
class Finder extends Object implements \IteratorAggregate
{
	...
	{
		if ($this->paths) {
			throw new Error('Directory to search has already been specified.');
		}
		if (!is_array($path)) {
			$path = func_get_args();
		}
	...
	{
		if (!$this->paths) {
			throw new Error('Call in() or from() to specify directory to search.');
	...
	private function buildIterator($path)
	{
		...
		if ($this->exclude) {
			...
			$iterator = new \Delorius\Iterators\RecursiveFilter($iterator, function($file) use ($filters) {
		...
		if ($this->groups) {
			$groups = $this->groups;
			$iterator = new \Delorius\Iterators\Filter($iterator, function($file) use ($groups) {
	...
	public function size($operator, $size = NULL)
	{
		if (func_num_args() === 1) { // in $operator is predicate
			if (!preg_match('#^(?:([=<>!]=?|<>)\s*)?((?:\d*\.)?\d+)\s*(K|M|G|)B?\z#i', $operator, $matches)) {
				throw new Error('Invalid size predicate format.');
			}
	...
	public function date($operator, $date = NULL)
	{
		if (func_num_args() === 1) { // in $operator is predicate
			if (!preg_match('#^(?:([=<>!]=?|<>)\s*)?(.+)\z#i', $operator, $matches)) {
				throw new Error('Invalid date predicate format.');
			}
