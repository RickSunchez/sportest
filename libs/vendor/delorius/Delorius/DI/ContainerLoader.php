...
use Delorius\Exception\IO;
use Delorius\Utils\FileSystem;

/**
 * DI container loader.
 */
class ContainerLoader extends Object
{
    /** @var bool */
    ...
    /**
     * @param  mixed
     * @param  callable  function (\Delorius\DI\Compiler $compiler): string|NULL
     * @return string
     */
    ...
    private function loadFile($class, $generator)
    {
        ...
        if (!$handle || !flock($handle, LOCK_EX)) {
            throw new IO("Unable to acquire exclusive lock on '$file.lock'.");
        }
        ...
        if (!is_file($file) || $this->isExpired($file)) {
            ...
            foreach ($toWrite as $name => $content) {
                if (file_put_contents("$name.tmp", $content) !== strlen($content) || !rename("$name.tmp", $name)) {
                    ...
                    throw new IO("Unable to create file '$name'.");
                }
            }
        }

        if ((@include $file) === FALSE) { // @ - error escalated to exception
            throw new IO("Unable to include '$file'.");
        }
