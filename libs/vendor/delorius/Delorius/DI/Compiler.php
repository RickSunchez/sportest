...
use Delorius\Exception\Deprecated;
...
class Compiler extends Object
{
    ...
    public function addExtension($name, CompilerExtension $extension)
    {
        if (isset($this->extensions[$name]) || isset(self::$reserved[$name])) {
            throw new InvalidArgument("Name '$name' is already used or reserved.");
        }
    ...
    {
        $this->config = \Delorius\DI\Config\Helpers::merge($config, $this->config);
        return $this;
    }
    ...
    /**
     * @return \Delorius\PhpGenerator\ClassType[]|string
     */
    ...
    {
        $last = $this->getExtensions('Delorius\DI\Extensions\InjectExtension');
        ...
        foreach ($first = $this->getExtensions('Delorius\DI\Extensions\ExtensionsExtension') as $name => $extension) {
        ...
        if ($extra = array_diff_key($this->extensions, $extensions, $first)) {
            ...
            throw new Deprecated("Extensions '$extra' were added while container was being compiled.");

        } elseif ($extra = array_diff_key($this->config, self::$reserved, $this->extensions)) {
            $extra = implode("', '", array_keys($extra));
            throw new InvalidState("Found sections '$extra' in configuration, but corresponding extensions are missing.");
        }
    }
    ...
    public static function parseServices(ContainerBuilder $builder, array $config, $namespace = NULL)
    {
        ...
        foreach ($services as $name => $def) {
            $path = array();
            while (\Delorius\DI\Config\Helpers::isInheriting($def)) {
                $path[] = $def;
                $def = isset($services[$def[\Delorius\DI\Config\Helpers::EXTENDS_KEY]]) ? $services[$def[\Delorius\DI\Config\Helpers::EXTENDS_KEY]] : array();
                if (in_array($def, $path, TRUE)) {
                    throw new Error("Circular reference detected for service '$name'.");
                }
            }
        ...
        foreach ($services as $origName => $def) {
            ...
            if (($parent = \Delorius\DI\Config\Helpers::takeParent($def)) && $parent !== $name) {
                $builder->removeDefinition($name);
                $definition = $builder->addDefinition(
                    $name,
                    $parent === \Delorius\DI\Config\Helpers::OVERWRITE ? NULL : unserialize(serialize($builder->getDefinition($parent))) // deep clone
                );
            ...
            try {
                ...
                throw new Error("Service '$name': " . $e->getMessage(), NULL, $e);
            }

            if ($definition->getClass() === 'self' || ($definition->getFactory() && $definition->getFactory()->getEntity() === 'self')) {
                throw new Deprecated("Replace service definition '$origName: self' with '- $origName'.");
            }
        }
    }
    ...
    public static function parseService(ServiceDefinition $definition, $config)
    {
        ...
        if ($error = array_diff(array_keys($config), $known)) {
            throw new InvalidState(sprintf("Unknown or deprecated key '%s' in definition of service.", implode("', '", $error)));
        }
        ...
        if (array_key_exists('class', $config)) {
            Validators::assertField($config, 'class', 'string|Delorius\DI\Statement|null');
        ...
        if (array_key_exists('create', $config)) {
            Validators::assertField($config, 'create', 'callable|Delorius\DI\Statement|null');
        ...
        if (isset($config['setup'])) {
            ...
            foreach ($config['setup'] as $id => $setup) {
                Validators::assert($setup, 'callable|Delorius\DI\Statement', "setup item #$id");
        ...
        if (isset($config['inject'])) {
            ...
            $definition->addTag(\Delorius\DI\Extensions\InjectExtension::TAG_INJECT, $config['inject']);
        }
        ...
        if (isset($config['tags'])) {
            ...
            if (\Delorius\DI\Config\Helpers::takeParent($config['tags'])) {
