...
use Delorius\PhpGenerator\Helpers as PhpHelpers;
...
class ContainerBuilder extends Object
{
	...
	/** @var \Delorius\PhpGenerator\ClassType[] */
	...
	public function addDefinition($name, ServiceDefinition $definition = NULL)
	{
		...
		if (isset($this->definitions[$name])) {
			throw new InvalidState("Service '$name' has already been added.");
		}
	...
	public function addAlias($alias, $service)
	{
		if (!is_string($alias) || !$alias) { // builder is not ready for falsy names such as '0'
			throw new InvalidArgument(sprintf('Alias name must be a non-empty string, %s given.', gettype($alias)));

		} elseif (!is_string($service) || !$service) { // builder is not ready for falsy names such as '0'
			throw new InvalidArgument(sprintf('Service name must be a non-empty string, %s given.', gettype($service)));

		} elseif (isset($this->aliases[$alias])) {
			throw new InvalidState("Alias '$alias' has already been added.");

		} elseif (isset($this->definitions[$alias])) {
			throw new InvalidState("Service '$alias' has already been added.");
	...
	public function getByType($class)
	{
		...
		if (empty($this->classes[$class][TRUE])) {
			...
			throw new ServiceCreation("Multiple services of type $class found: " . implode(', ', $this->classes[$class][TRUE]));
		}
	}
	...
	public function autowireArguments($class, $method, array $arguments)
	{
		...
		if (!$rc->hasMethod($method)) {
			if (!Arrays::isList($arguments)) {
				throw new ServiceCreation("Unable to pass specified arguments to $class::$method().");
			}
			return $arguments;
		}
		...
		if (!$rm->isPublic()) {
			throw new ServiceCreation("$class::$method() is not callable.");
		}
	...
	public function prepareClassList()
	{
		...
		$this->addDefinition(self::THIS_CONTAINER)->setClass('Delorius\DI\Container');
		...
		foreach ($this->definitions as $name => $def) {
			...
			if ($def->isDynamic()) {
				if (!$def->getClass()) {
					throw new ServiceCreation("Class is missing in definition of service '$name'.");
				}
				$def->setFactory(NULL);
				continue;
			}
			...
			if (!$def->getEntity()) {
				if (!$def->getClass()) {
					throw new ServiceCreation("Class and factory are missing in definition of service '$name'.");
				}
	...
	private function resolveImplement(ServiceDefinition $def, $name)
	{
		...
		if (!interface_exists($interface)) {
			throw new ServiceCreation("Interface $interface used in service '$name' not found.");
		}
		...
		if (count($rc->getMethods()) !== 1 || !$method || $method->isStatic()) {
			throw new ServiceCreation("Interface $interface used in service '$name' must have just one non-static method create() or get().");
		}
		...
		if (!$def->getClass() && !$def->getEntity()) {
			...
			if (!$returnType) {
				throw new ServiceCreation("Method $interface::$methodName() used in service '$name' has no @return annotation.");
			} elseif (!class_exists($returnType)) {
				throw new ServiceCreation("Please check a @return annotation of the $interface::$methodName() method used in service '$name'. Class '$returnType' cannot be found.");
			}
		...
		if ($methodName === 'get') {
			if ($method->getParameters()) {
				throw new ServiceCreation("Method $interface::get() used in service '$name' must have no arguments.");
			}
			if (!$def->getEntity()) {
				$def->setFactory('@\\' . ltrim($def->getClass(), '\\'));
			} elseif (!$this->getServiceName($def->getFactory()->getEntity())) {
				throw new ServiceCreation("Invalid factory in service '$name' definition.");
			}
		}
			...
			foreach ($method->getParameters() as $param) {
				...
				if (isset($ctorParams[$param->getName()])) {
					...
					if ($hint !== PhpReflection::getParameterType($arg)) {
						throw new ServiceCreation("Type hint for \${$param->getName()} in $interface::$methodName() doesn't match type hint in $class constructor.");
					}
	...
	private function resolveServiceClass($name, $recursive = array())
	{
		if (isset($recursive[$name])) {
			throw new ServiceCreation(sprintf('Circular reference detected for services: %s.', implode(', ', array_keys($recursive))));
		}
		$recursive[$name] = TRUE;
		...
		if ($class = $def->getClass() ?: $class) {
			...
			if (!class_exists($class) && !interface_exists($class)) {
				throw new ServiceCreation("Type $class used in service '$name' not found or is not class or interface.");
			}
			self::checkCase($class);
	...
	private function resolveEntityClass($entity, $recursive = array())
	{
		...
		if (is_array($entity)) {
			...
			if (isset($e) || ($refClass && (!$reflection->isPublic()
				...
				throw new ServiceCreation(sprintf("Factory '%s' used in service '%s' is not callable.", Callback::toString($entity), $name[0]));
			}
			...
			if (!class_exists($entity) || !($rc = new \ReflectionClass($entity)) || !$rc->isInstantiable()) {
				$name = array_slice(array_keys($recursive), -1);
				throw new ServiceCreation("Class $entity used in service '$name[0]' not found or is not instantiable.");
			}
			return ltrim($entity, '\\');
		}
	}
	...
	private function checkCase($class)
	{
		if (class_exists($class) && ($rc = new \ReflectionClass($class)) && $class !== $rc->getName()) {
			throw new ServiceCreation("Case mismatch on class name '$class', correct name is '{$rc->getName()}'.");
		}
	}
	 ...
	 * @return \Delorius\PhpGenerator\ClassType[]
	 */
	public function generateClasses($className = NULL, $parentName = NULL)
	{
		...
		$containerClass = $this->generatedClasses[] = new \Delorius\PhpGenerator\ClassType($this->className);
		$containerClass->setExtends($parentName ?: 'Delorius\DI\Container');
		...
		foreach ($definitions as $name => $def) {
			try {
				...
				if (!PhpHelpers::isIdentifier($methodName)) {
					throw new ServiceCreation('Name contains invalid characters.');
				}
				...
				throw new ServiceCreation("Service '$name': " . $e->getMessage(), NULL, $e);
			}
		}
	...
	private function generateService($name)
	{
		...
		if ($def->isDynamic()) {
			return PhpHelpers::formatArgs('throw new Delorius\\DI\\ServiceCreation(?);',
		...
		if (($class = $def->getClass()) && !$serviceRef && $class !== $entity
			...
			$code .= PhpHelpers::formatArgs("if (!\$service instanceof $class) {\n"
				. "\tthrow new Delorius\\Exception\\Error(?);\n}\n",
		...
		$factoryClass = $this->generatedClasses[] = new \Delorius\PhpGenerator\ClassType;
	 ...
	 * @return \Delorius\PhpGenerator\Parameter[]
	 */
	private function convertParameters(array $parameters)
	{
		...
		foreach ($parameters as $k => $v) {
			...
			$param = $res[] = new \Delorius\PhpGenerator\Parameter;
	...
	public function formatStatement(Statement $statement)
	{
		...
		if (is_string($entity) && Strings::contains($entity, '?')) { // PHP literal
			...
			if ($constructor = $rc->getConstructor()) {
				...
				throw new ServiceCreation("Unable to pass arguments, class $entity has no constructor.");
			}
			return $this->formatPhp("new $entity" . ($arguments ? '(?*)' : ''), array($arguments));

		} elseif (!Arrays::isList($entity) || count($entity) !== 2) {
			throw new ServiceCreation(sprintf('Expected class, method or property, %s given.', PhpHelpers::dump($entity)));

		} elseif (!preg_match('#^\$?' . PhpHelpers::PHP_IDENT . '\z#', $entity[1])) {
			throw new ServiceCreation("Expected function, method or property name, '$entity[1]' given.");
			...
			Validators::assert($arguments, 'list:1', "setup arguments for '" . Callback::toString($entity) . "'");
	...
	/**
	 * @return \Delorius\PhpGenerator\PhpLiteral
	 */
	public static function literal($phpCode)
	{
		return new \Delorius\PhpGenerator\PhpLiteral($phpCode);
	}
	...
	public function getServiceName($arg)
	{
		...
		if (Strings::contains($service, '\\')) {
			...
			if (!$res) {
				throw new ServiceCreation("Reference to missing service of type $service.");
			}
			return $res;
		}
		$service = isset($this->aliases[$service]) ? $this->aliases[$service] : $service;
		if (!isset($this->definitions[$service])) {
			throw new ServiceCreation("Reference to missing service '$service'.");
		}
		return $service;
	}
