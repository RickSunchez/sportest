 ...
 */
class IniAdapter extends Object implements IAdapter
{
	/** @internal */
	...
	public function load($file)
	{
		...
		if ($ini === FALSE) {
			...
			throw new InvalidState("parse_ini_file(): $error[message]");
		}
		...
		foreach ($ini as $secName => $secData) {
					...
					foreach ($secData as $key => $val) {
						...
						foreach (explode(self::KEY_SEPARATOR, $key) as $part) {
							...
							if (!isset($cursor[$part]) || is_array($cursor[$part])) {
								...
								throw new InvalidState("Invalid key '$key' in section [$secName] in file '$file'.");
							}
						}
						$cursor = $val;
					}
			...
			foreach (explode(self::KEY_SEPARATOR, $secName) as $part) {
				if (!isset($cursor[$part]) || is_array($cursor[$part])) {
					...
					throw new InvalidState("Invalid section [$secName] in file '$file'.");
				}
			}
	...
	private static function build($input, & $output, $prefix)
	{
		foreach ($input as $key => $val) {
			...
			if (is_array($val)) {
				...
				throw new InvalidArgument(sprintf("The '%s' item must be scalar or array, %s given.", $prefix . $key, gettype($val)));
			}
		}
	}
